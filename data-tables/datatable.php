...
// code to pull/push data to examination details

if($_GET['type']=="examsearch")
	{
		$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.branch','s.contact','s.joindate');
	...
	$sWhere = " WHERE b.id=s.branch and s.delete_status='0' and s.balance<100000 ";
	if ( isset($_GET['sSearch'])&& $_GET['sSearch'] != "" )
	{
	...
	/* Total data set length */
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.id=s.branch and s.delete_status='0' and s.balance<100000  ";
	$rResultTotal = $conn->query( $sQuery ) or die(mysqli_error($conn));
	$aResultTotal = $rResultTotal->fetch_assoc();
	$iTotal = $aResultTotal['cc'];
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
					...
					html_entity_decode('<button class="btn btn-success btn-xs" onclick="javascript:GetExamForm('.$aRow['id'].')"><i class="fa fa-graduation-cap "></i> Process Score</button>')
...
// salary processing
if($_GET['type']=="salarysearch")
	{
		$aColumns = array( 's.id','s.iname','s.idno','s.salary','b.branch','s.contact','s.joindate');
			
			/* Indexed column (used for fast and accurate table cardinality) */
			$sIndexColumn = "s.id";
			
			/* DB table to use */
			$sTable = " staff as s,branch as b ";	
		/* 
		* Paging
		*/
		$sLimit = "";
	...
	$sWhere = " WHERE b.id=s.branch and s.delete_status='0' and s.salary>0 ";
	if ( isset($_GET['sSearch'])&& $_GET['sSearch'] != "" )
	{
	...
	/* Total data set length */
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.id=s.branch and s.delete_status='0' and s.salary>0  ";
	$rResultTotal = $conn->query( $sQuery ) or die(mysqli_error($conn));
	$aResultTotal = $rResultTotal->fetch_assoc();
	$iTotal = $aResultTotal['cc'];
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
                    html_entity_decode($aRow['iname'].'<br/>'.$aRow['contact']),
                    $aRow['idno'],
					$aRow['salary'],
                    $aRow['branch'],
					date("d M y", strtotime($aRow['joindate'])),
                    
					html_entity_decode('<button class="btn btn-info btn-xs" onclick="javascript:GetSalaryForm('.$aRow['id'].')"><i class="fa fa-euro "></i>  Process Salary </button>')
